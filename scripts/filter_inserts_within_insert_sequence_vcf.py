...
parser.add_argument('--suffix', default=".all.read_insertions.repbase_annotated.mapq_ct_filtered.tsv")
...
parser.add_argument('--ref-sample', default="DDTS_0001_nn_n")
...
for sample in args.sample.split(','):
    with open("./"+sample+"/"+args.merged_folder+"/"+sample+args.merged_reads) as in_mr:
        for line in in_mr:
...
def get_mapq_reads(samples, regions, mapq_reads, ref_sample):
    records_per_read = defaultdict(list)
    for sample in samples.split(','):
...
if threads_to_use > 1:
    ...
    for reg in regions_split:
        async_result = pool.apply_async(get_mapq_reads, (args.sample, regions_split[reg], low_mapq, args.ref_sample))
    ...
    for reg in regions_split:
        results_mapq.append(get_mapq_reads(args.sample, regions_split[reg], low_mapq, args.ref_sample))
    for i in results:
        for read in i:
